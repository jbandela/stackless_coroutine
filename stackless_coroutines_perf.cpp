...
auto do_coroutine(boost::asio::io_service &io, std::string host,
		...
		auto pval = std::make_unique<val>(io);
 
	auto tuple = 
 stackless_coroutine::make_block(

      [](auto &context, auto &value,std::string host, std::string port, int size) {
      ...
      stackless_coroutine::make_while_true(
          ...
          [](auto &context, auto &value) {
            value.socket_.async_read_some(
                boost::asio::buffer(value.current.data(),
                                    value.current.size()),
                context);
            return context.do_async();
	...
	return stackless_coroutine::run(std::move(pval), tuple, std::move(f),host,port,size);
...
int main(int argc, char** argv) {
  ...
  auto func = [&]() {
    do_coroutine( io, "127.0.0.1", argv[1],std::stoi(argv[2]),[&](auto &value, std::exception_ptr e,  auto op) {
      if (e) {
        done_promise.set_exception(e);
      } else {
		  auto end_time = std::chrono::steady_clock::now();

		  auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - value.start_time);
		  std::cout << "Received " << value.bytes << " in " << duration.count() << " milliseconds\n";
